...
int main() {
    ...
    if (id1 < 0) {
        ...
        char pac[3]; // строка по 3 чара
        sprintf(pac, "%d", ac[0]); // a - > c, программа с читает
        ...
        if (id2 < 0) {
            ...
            while ((read_string(&str, STDIN)) > 0) { // читаем по строке до ctrl d
                ...
                int ok; // стопим пока С не пришлёт ок
                read(ca[0], &ok, sizeof(ok));  // стопается пока туда что-то запишут
